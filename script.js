...
async function init() {
    try {
        ...
        // 使用 Teachable Machine 模型 - 使用絕對路徑
        const modelURL = window.location.origin + '/model.json';
        const metadataURL = window.location.origin + '/metadata.json';
...
async function startCamera() {
    ...
    try {
        ...
        document.getElementById('captureCanvas').parentNode.appendChild(webcam.canvas);
...
function displayResults(predictions, containerId) {
    ...
    const normalizedPredictions = predictions.map((pred, index) => {
        ...
        return {
            ...
            className: pred.className || labels[index]
        };
    });
    ...
    normalizedPredictions.forEach((prediction) => {
        ...
        const labelInfo = labelNames[label];
    ...
    if (topPrediction && topPrediction.confidence > 0) {
        ...
        const topLabelInfo = labelNames[topPrediction.label];
...
async function stopCamera() {
    ...
    if (webcam) {
        ...
        canvases.forEach(canvas => {
            if (canvas !== document.getElementById('captureCanvas')) {
                canvas.remove();
            }
        });
    }
